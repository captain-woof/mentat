...
class SSHProxyManager:
    ...
    def start_tunnels(self):
        ...
        for config in self.ssh_configs:
            ...
            try:
                ...
                print(f"Started SOCKS5 proxy via {config['ip']} on 127.0.0.1:{local_port} (PID: {process.pid})")

            except (FileNotFoundError, RuntimeError, TimeoutError, Exception) as e:
                print(f"Failed to start tunnel for {config['ip']}: {e}")
    ...
    def stop_tunnels(self):
        ...
        self.processes = []
        self.proxies = []
        print("All SSH proxy processes have been stopped.")
